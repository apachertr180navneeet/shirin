<div class="shop__sidebar--widget widget__area product_d_widget">
    <div class="single__widget widget__bg">
        <h2 class="widget__title h3">{{ translate('Top Selling Products') }}</h2>
        <div class="shop__sidebar--product">
            ...
            <div class="small__product--card d-flex">
                <div class="small__product--thumbnail">
                    <a class="display-block" href="{{ route('product', $top_product->slug) }}"><img class="product__media--preview__items--img"
                             src="{{ uploaded_asset($top_product->thumbnail_img) }}"
                             onerror="this.src='{{ static_asset('public/assets/img/placeholder.jpg') }}'"></a>
                </div>
                <div class="small__product--content">
                    <h3 class="small__product--card__title"><a href="{{ route('product', $top_product->slug) }}">{{ $top_product->getTranslation('name') }}</a></h3>
                    <div class="small__product--card__price mb_5">
                        <span class="current__price">{{ home_discounted_base_price($top_product) }}</span>
                    ...
                    <ul class="rating small__product--rating d-flex">
                        @for ($i=0; $i < $top_product->review_rating; $i++)
                        ...
                        @endfor
                        @for ($i=0; $i < 5-$top_product->review_rating; $i++)
                            <li class="rating__list">
                                <span class="rating__icon">
                                    <svg width="12" height="11" viewBox="0 0 14 13" fill="none" xmlns="http://www.w3.org/2000/svg">
