<div class="modal-body px-4 py-5 c-scrollbar-light">
    ...
    <div class="media mb-1">
        <img src="{{ static_asset('public/assets/img/placeholder.jpg') }}" data-src="{{ uploaded_asset($product->thumbnail_img) }}"
            class="bg__gray mr-4 lazyload size-90px img-fit rounded-3 border p-2" alt="Product Image" onerror="this.onerror=null;this.src='{{ static_asset('public/assets/img/placeholder.jpg') }}';">
        ...
        <div class="p-3">
            <div class="aiz-carousel gutters-5 half-outside-arrow" data-items="2" data-xl-items="3" data-lg-items="4"  data-md-items="3" data-sm-items="2" data-xs-items="2" data-arrows='true' data-infinite='true'>
                ...
                <div class="carousel-box hov-scale-img hov-shadow-sm">
                    <div class="product__card aiz-card-box my-2 has-transition">
                        <div class="product__card--thumbnail">
                            <a href="{{ route('product', $related_product->slug) }}" class="product__card--thumbnail__link display-block">
                                <img class="product__card--thumbnail__img img-fit lazyload mx-auto h-140px h-md-200px has-transition"
                        ...
                        <div class="product__card--content p-md-3 p-2 text-center">
                            <h3 class="product__card--title text-truncate-2 lh-1-4 mb-0 h-35px">
                                <a href="{{ route('product', $related_product->slug) }}" class="product__card--title">{{ $related_product->getTranslation('name') }}</a>
                            </h3>
                            <div class="fs-14 mt-3">
                                <span class="current__price fw-700 ">{{ home_discounted_base_price($related_product) }}</span>
                                @if(home_base_price($related_product) != home_discounted_base_price($related_product))
                                    <span class="old__price fw-600 opacity-50 ml-1">{{ home_base_price($related_product) }}</span>
                                @endif
                            </div>
                        </div>
                    </div>
    ...
    <div class="row gutters-5">
        <div class="col-sm-6">
            <button class="primary__btn mb-3 mb-sm-0 btn-block rounded-5 text-white" data-dismiss="modal">{{ translate('Back to shopping')}}</button>
        </div>
        <div class="col-sm-6">
            <a href="{{ route('cart') }}" class="primary__btn mb-3 mb-sm-0 btn-block rounded-5 text-center text-uppercase">{{ translate('Proceed to Checkout')}}</a>
        </div>
