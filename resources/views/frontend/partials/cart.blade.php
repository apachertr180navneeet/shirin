@php
    ...
    if(isset($cart) && count($cart) > 0){
        foreach ($cart as $key => $cartItem){
            ...
            $total1 = $total + cart_product_price($cartItem, $product, false) * $cartItem['quantity'];
        }
    }
@endphp
<span class="items__count cart-count">   
    {{count($cart)}}                                  
</span> 
{{-- <div class="header-cart iconset" title="Cart">
    <a href="{{url('cart')}}" class="header-cart btn-minicart clr-none"><i class="hdr-icon icon anm anm-cart-l"></i><span class="cart-count">{{count($cart)}}</span></a>
</div> --}}
                    ...
                    <li class="list-group-item border-0 hov-scale-img">
                        <span class="d-flex align-items-center">
                            <a href="{{ route('product', $product->slug) }}"
                                ...
                                <img src="{{ static_asset('public/assets/img/placeholder.jpg') }}"
