<div class="card rounded-5 border single__widget widget__bg">
    ...
    <div class="card-header pt-4 pb-1 border-bottom-0">
        ...
        <div class="text-right">
            <!-- Items Count -->
            <span class="badge-inline badge-primary fs-12 rounded-0 px-2" style="background: var(--secondary-color)">
                {{ count($carts) }}
                {{ translate('Items') }}
            </span>
        ...
        <table class="table">
            ...
            <tbody>
                @php
                    ...
                    $shipping_region = data_get($shipping_info, 'city');
                @endphp
                @foreach ($carts as $key => $cartItem)
                    @php
                        $product = \App\Models\Product::find($cartItem['product_id']);
                    ...
                    <tr class="cart_item">
                        ...
                        <td class="product-total text-right pr-0 fs-14 current__price fw-600 border-top-0 border-bottom">
                            <span
        ...
        <table class="table" style="margin-top: 2rem!important;">
            <tfoot>
                ...
                <tr class="cart-subtotal">
                    ...
                    <td class="text-right pr-0 fs-14 pt-0 pb-2 fw-600 current__price border-top-0">
                ...
                <tr class="cart-shipping">
                    ...
                    <td class="text-right pr-0 fs-14 pt-0 pb-2 fw-600 current__price current__price border-top-0">
                ...
                @php
                    ...
                    // Promotion: remove lowest unit-priced item(s) based on total item quantity.
                    $unit_prices = [];
                    foreach ($carts as $cartItem) {
                        $unit_price = cart_product_price($cartItem, $cartItem->product, false, false);
                    ...
                    $lowest_price_discount = 0;
                    $item_count = count($unit_prices);
                    if ($item_count > 2 && isset($unit_prices[0])) {
                        $lowest_price_discount += $unit_prices[0];
                    }
                    if ($item_count > 3 && isset($unit_prices[1])) {
                        $lowest_price_discount += $unit_prices[1];
                    }

                    $total -= $lowest_price_discount;
                    if (Session::has('club_point')) {
                ...
                @endphp
                @if ($lowest_price_discount > 0)
                    <tr class="cart-shipping">
                        <th class="pl-0 fs-14 pt-0 pb-2 text-dark fw-600 border-top-0">{{ translate('Price Discount') }}</th>
                        <td class="text-right pr-0 fs-14 pt-0 pb-2 fw-600 current__price border-top-0">
                            <span class="fw-600">-{{ single_price($lowest_price_discount) }}</span>
                        </td>
                    </tr>
                @endif
                <!-- Total -->
                <tr class="cart-total">
                    ...
                    <td class="text-right pr-0 fs-14 fw-600 current__price">
            ...
            @if ($coupon_discount > 0 && $coupon_code)
                <div class="mt-3">
                    <form class="" id="remove-coupon-form" enctype="multipart/form-data">
                        @csrf
                        <div class="input-group">
                            <div class="account__login--input rounded-0">{{ $coupon_code }}</div>
                            <!--<div class="input-group-append">-->
                                <button type="button" id="coupon-remove"
                                    class="primary__btn rounded-0 w-100">{{ translate('Change Coupon') }}</button>
                            <!--</div>-->
                        </div>
                    </form>
                </div>
            @else
                <div class="mt-3">
                    <form class="" id="apply-coupon-form" enctype="multipart/form-data">
                        ...
                        <div class="input-group">
                            <input type="text" class="account__login--input rounded-0" name="code"
                                onkeydown="return event.key != 'Enter';"
                                placeholder="{{ translate('Have coupon code? Apply here') }}" required>
                            <!-- <div class="input-group-append"> -->
                                <button type="button" id="coupon-apply"
                                    class="primary__btn rounded-0 w-100">{{ translate('Apply') }}</button>
                            <!-- </div> -->
                        </div>
                    </form>
                </div>
