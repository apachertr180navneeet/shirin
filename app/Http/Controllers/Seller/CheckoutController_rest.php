...
//        if (Session::has('cart') && count(Session::get('cart')) > 0) {
    ...
    {
        if ($request->address_id == null || $request->address_id == 0) {
            flash(translate("Please add shipping address"))->warning();
            return back();
        }

        if (Address::find($request->address_id) == null) {
            flash(translate("Please select a valid shipping address"))->warning();
            return back();
        }
        ...
        if(get_setting('shipping_type') == 'carrier_wise_shipping'){
            ...
            $zone = \App\Models\Country::where('id', $address->country_id)->first()->zone_id;
    ...
    public function store_delivery_info(Request $request)
    {
        ...
        if ($request->has('address_id')) {
            ...
            // Re-fetch carts to ensure the address_id is updated in the collection
            $carts = Cart::where('user_id', Auth::user()->id)->get();
        }

        if ($carts[0]['address_id'] == null || $carts[0]['address_id'] == 0) {
            flash(translate('Please select a shipping address'))->warning();
            return redirect()->route('checkout.shipping_info');
        }
