...
(function ($) {
    ...
    AIZ.plugins = {
        metismenu: function () {
            if ($('[data-toggle="aiz-side-menu"]').length > 0 && typeof $.fn.metisMenu === 'function') {
        ...
        tagify: function () {
            if (typeof $.fn.tagify === 'undefined') return;
        ...
        fooTable: function () {
            if (typeof $.fn.footable === 'function') {
                ...
                // Fallback: show tables if footable CDN fails to load
                $(".aiz-table").css({ 'opacity': 1, 'height': 'auto' });
            }
        },
        notify: function (type = "dark", message = "") {
            // Check if $.notify exists
            if (typeof $.notify === 'undefined') {
                console.warn('Bootstrap Notify plugin not loaded. Falling back to alert.');
                alert(message);
                return;
            }
        ...
        tooltip: function () {
            ...
            try {
                ...
                console.error('Tooltip error:', e.message);
            }
        },
        countDown: function () {
            if (typeof $.fn.countdown === 'undefined') return;
        ...
        jsSocials: function(){
            if (typeof $.fn.jsSocials === 'undefined') return;
            if($('.aiz-share')[0]){
                $('.aiz-share').jsSocials({
                    showLabel: false,
                    showCount: false,
